...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      url: `http://39.96.77.250/view/readBook?&bookId=${this.data.bookId}&chapterId=${this.data.chapterId}`, 
      method: 'POST',
      header: {
      ...
      success: function (res) {
        ...
        //console.log(this.data.Text)
         this.setData({
           Text: res.data.result[1].content,
  ...
  toMl() {  //触发目录
    ...
    wx.navigateTo({
      url: `/pages/bookML/bookML?id=${this.data.bookId}`,
      success: (result) => {
        wx.hideLoading();
      }
    });
